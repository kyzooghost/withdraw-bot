...
// ApplyOverrides applies stored threshold overrides to a Morpho monitor module,
// returning the module with updated thresholds. Non-Morpho modules pass through unchanged.
func (f *Factory) ApplyOverrides(module core.MonitorModule, overrides []core.ThresholdOverride, assetDecimals uint8) (core.MonitorModule, error) {
...
// ValidateThresholdChange validates a proposed override value and, if module config
// is available, checks that the effective thresholds remain consistent.
func (f *Factory) ValidateThresholdChange(ctx context.Context, moduleID string, key string, value string, moduleConfig config.ModuleConfig, overrides []core.ThresholdOverride, assetDecimals uint8) error {
	if err := validateThresholdValue(moduleID, key, value, assetDecimals); err != nil {
		return err
	}
	if moduleConfig == nil {
		return nil
	}
	id := core.MonitorModuleID(moduleID)
	overrides = upsertOverride(overrides, moduleID, key, value)
	...
	case core.ModuleWithdrawLiquidity:
		module, err := withdrawLiquidityThresholdConfig(moduleConfig, assetDecimals)
		if err != nil {
			return err
		}
		effective, err := applyWithdrawLiquidityOverrides(module, overrides, assetDecimals)
		if err != nil {
			return err
		}
...
func upsertOverride(overrides []core.ThresholdOverride, moduleID string, key string, value string) []core.ThresholdOverride {
	result := append([]core.ThresholdOverride{}, overrides...)
	for index, override := range result {
		if override.ModuleID == moduleID && override.Key == key {
			result[index].Value = value
			return result
		}
	}
	return append(result, core.ThresholdOverride{ModuleID: moduleID, Key: key, Value: value})
}

func applySharePriceOverrides(module morphomod.SharePriceModule, overrides []core.ThresholdOverride) (core.MonitorModule, error) {
...
func applyWithdrawLiquidityOverrides(module morphomod.WithdrawLiquidityModule, overrides []core.ThresholdOverride, assetDecimals uint8) (core.MonitorModule, error) {
...
func applyVaultStateOverrides(module morphomod.VaultStateModule, overrides []core.ThresholdOverride) (core.MonitorModule, error) {
...
func overridesForModule(overrides []core.ThresholdOverride, moduleID core.MonitorModuleID) []core.ThresholdOverride {
	result := make([]core.ThresholdOverride, 0, len(overrides))
...
// Noop readers for validation-only module construction (no real RPC calls needed).

type noopSharePriceReader struct{}

func (noopSharePriceReader) CurrentSharePrice(_ context.Context) (*big.Int, error) {
	return big.NewInt(1), nil
}

type noopIdleAssetReader struct{}

func (noopIdleAssetReader) IdleAssets(_ context.Context, _ common.Address) (*big.Int, error) {
	return big.NewInt(1), nil
}

type noopExitSimulator struct{}

func (noopExitSimulator) Position(_ context.Context) (core.PositionSnapshot, error) {
	return core.PositionSnapshot{}, nil
}

func (noopExitSimulator) SimulateFullExit(_ context.Context, _ core.FullExitRequest) (core.FullExitSimulation, error) {
	return core.FullExitSimulation{}, nil
}

type noopVaultStateReader struct{}

func (noopVaultStateReader) CurrentVaultState(_ context.Context) (morphomod.VaultStateSnapshot, error) {
